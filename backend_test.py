...
class CIRSBackendTester:
    ...
    def test_issue_creation(self):
        ...
        # Add user_id to the issue data as the frontend does
        issue_with_image_and_user = issue_with_image.copy()
        issue_with_image_and_user["user_id"] = self.test_user["id"]
        
        try:
            response = self.session.post(f"{self.base_url}/issues", 
                                       json=issue_with_image_and_user, 
                                       timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
                if data.get("success") and "issue" in data:
    ...
    def test_voting_system(self):
        ...
        # Test adding a vote
        try:
            vote_data = {"user_id": self.test_user["id"]}
            response = self.session.post(f"{self.base_url}/issues/{issue_id}/vote", 
                                       json=vote_data, timeout=TIMEOUT)
            if response.status_code == 200:
                data = response.json()
